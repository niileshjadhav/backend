...
class RegionConfig(Base):
    ...
    password = Column(Text, nullable=False)  # Use Text for encrypted/longer passwords
    ...
    def __repr__(self):
        return f"<RegionConfig(region='{self.region}', host='{self.host}', is_active={self.is_active})>"
    
    def get_database_url(self) -> str:
        """Generate database URL from stored configuration"""
    ...
    def to_dict_secure(self) -> dict:
        """Convert to dictionary without password for secure API responses"""
        result = self.to_dict()
        result.pop("password", None)  # Remove password from response
        return result